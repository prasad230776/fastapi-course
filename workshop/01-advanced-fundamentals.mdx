...
7. Python Nuances
   ...
   - Conditional Expressions (One-line `if-else`)
   - Chained Comparisons
...
Logical operators in Python (`and` and `or`) use **short-circuit evaluation**. This means Python only evaluates as much of a logical expression as is necessary to determine the final result, and returns the actual value of the evaluated operand (not just a boolean `True` or `False`).

### Truthy vs Falsy Values
In Python, every object has an inherent boolean value when used in a conditional context:
...
- **Truthy Values:**
  - Any non-zero number (e.g., `3`, `-1`)
  - Any non-empty sequence or collection (e.g., `"zero"`, `[1, 2]`)
  - `True`

---

### How `or` and `and` Work
- **`A or B`**: Evaluates `A`. If `A` is Truthy, it returns `A` immediately (short-circuits) without evaluating `B`. If `A` is Falsy, it evaluates and returns `B`.
- **`A and B`**: Evaluates `A`. If `A` is Falsy, it returns `A` immediately (short-circuits) without evaluating `B`. If `A` is Truthy, it evaluates and returns `B`.

Let's look at how Python evaluates the following expressions:

#### 1. What is the output of `3 or "zero"`?
Since `3` is a non-zero integer, it is Truthy. The `or` operator returns the first operand immediately.
...
#### 2. What is the output of `3 and "zero"`?
`3` is Truthy. For `and` to succeed, it must evaluate the second operand. It returns `"zero"`.
...
#### 3. What is the output of `"" and 3`?
The empty string `""` is Falsy. Since `and` requires both operands to be truthy, it short-circuits on the first falsy operand and returns `""`.
...
#### 4. What is the output of `"" or 3`?
`""` is Falsy. The `or` operator must check the second operand. It evaluates and returns `3`.
...
#### Fallback / Default Values (`or`)
```python
username = ""
# If username is empty/falsy, fallback to "Guest"
...
```python
logged_in = True
# Only runs the print if logged_in is True
logged_in and print("Welcome!")  # Output: Welcome!
```

### Exercise 1
Write a program to print `"Anonymous"` whenever `name` is an empty string using the `or` operator.
...
### Exercise 2
Print `"Access Granted"` only when `is_admin` is `True` using the `and` operator.
