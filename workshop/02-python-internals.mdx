...
1. **Variable Scope and resolution** (LEGB rule, `global` and `nonlocal` keywords)
2. **Function Arguments**: Positional vs Keyword argument passing
3. **Print formatting**: `sep` and `end` parameters
4. **Packing and Unpacking**: `*` and `**` operators, `*args`, and `**kwargs`
5. **Mutable Default Arguments Pitfall**
